...
import { AppleIcon, GoogleIcon } from "../../register/create-entry/icons";
...
export const SettingsAccountItem: FunctionComponent = observer(() => {
  ...
  const metadata = keyRingStore.multiKeyStoreInfo
    .filter((keyStore) => {
      return keyStore.type === "mnemonic" || keyStore.type === "privateKey";
    })
    .map((keyStore) => {
    ...
    .shift();
  const walletName =
    metadata?.name ?? intl.formatMessage({ id: "settings.text.myWallet" });
