...
You can determine whether Keplr is installed on the user device by checking `window.astra`. If `window.astra` returns `undefined` after document.load, Keplr is not installed. There are several ways to wait for the load event to check the status. Refer to the examples below:
...
window.onload = async () => {
    if (!window.astra) {
        ...
        const offlineSigner = window.astra.getOfflineSigner(chainId);
...
The `window.astra.enable(chainIds)` method requests the extension to be unlocked if it's currently locked. If the user hasn't given permission to the webpage, it will ask the user to give permission for the webpage to access Keplr.
